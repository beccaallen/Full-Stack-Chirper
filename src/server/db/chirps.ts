...
const all = async () => Query("SELECT chirps.id, chirps.content, chirps.location, chirps._created, users.name FROM chirps JOIN users on users.id=chirps.userid ORDER BY chirps._created DESC;");
